...
```sh
cp -r devbox/templates/git/default.local.example devbox/templates/git/default.local
```

## How resolution works

When `devbox render git` reads a template file declared in
`devbox/templates/git/default/manifest.yml`, it first checks the same relative
path under `devbox/templates/git/default.local/`:

- file present → that file is used and the renderer prints
  `using local override: devbox/templates/git/default.local/<rel>`
...
- Re-running `devbox render git` always resets file mode to `0755`.
