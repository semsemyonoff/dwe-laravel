...
- `internal/usercommands/` — declarative command system facade (re-exports all public symbols from subpackages: `model/`, `loader/`, `registry/`, `resolve/`, `runtime/`). Primary API: `CommandFile`, `Registry`, `LoadRegistry`, `RunCommand`, `ConfirmCommand`, `RunContext`, `HostRunner`, `DevboxRunner`, `ServiceExecRunner`, `ServiceRunRunner`, `ScriptRunner`, `WorkflowRunner`, param/context resolution, `${...}` template sugar. Script contract env: `DEVBOX_BIN` (path to current devbox binary), `DEVBOX_FILES_JSON` (JSON object mapping file IDs to `{path}`), `DEVBOX_NONINTERACTIVE` (set when running non-interactively via `--yes` flag)
...
- Confirmation routing (command-level `Confirmation: true`, builtin `confirm` step, workflow confirm steps): four-way dispatch — (1) `SkipConfirm` flag set → no-op; (2) `ConfirmFunc` injected → callback (tests); (3) `ui.IsInteractiveFn(stdin)` true → `ui.RunConfirm` (huh TTY form, `ErrCancelled` on Esc maps to "aborted by user"); (4) otherwise → plain stdin Y/n fallback (CI/pipe). `ExecContext.Stdin` and `RunContext.Stdin` override `os.Stdin` for tests. The `devbox commands run --yes` / `-y` flag sets tier 1 (SkipConfirm) and propagates `DEVBOX_NONINTERACTIVE=1` to subprocess env for non-interactive nested calls.
- Lifecycle pipeline (`devbox run` / `devbox stop`): separate YAML (`devbox/lifecycle.yml`), separate loader (`LoadLifecycleConfig`), reuses the same pipeline executor (`pipeline.Run`, `pipeline.ResolvePhaseSteps`, `pipeline.PlainReporter`). `continue_on_error: true` on a step lets hook phases fail without aborting the main sequence. `devbox run` layered update-mode precedence: `--no-update` flag > `--update <mode>` flag > `EffectiveMode()` from YAML. A successful `git pull --ff-only` (HEAD moved) triggers in-process reload of `DevboxConfig`, `LifecycleConfig`, and command registry before phase execution.
...
Commands may declare a `files:` block to handle external file artefacts (read or write). Each file entry specifies: access mode (`read`, `write`, `read_write`), path or candidates (with glob+match+sort fallback for read modes), and optional env injection. The CLI resolves paths (with template rendering, relative-path normalization to absolute, mkdir for write), exposes resolved paths as `${files.<id>.path}` template variables and env vars, and enforces safe-cleanup semantics: `on_error: remove` deletes files created by the invocation only (not pre-existing files). Templates in `path`, `candidates`, `confirmation_text`, `run`, `workdir`, and `env:` can reference `${files.<id>.path}` after computation but before user confirmation. File ID grammar: `[a-zA-Z_][a-zA-Z0-9_]*` (letters, digits, underscore; no hyphens or dots). See the declarative command definitions in `devbox/commands/` for usage examples (e.g. `db.dump-create`, `db.dump-deploy`).

### Commands System Features

**Workflow step conditions and error handling**: Workflow steps support two new optional fields. `when: <expr>` skips a step if the expression evaluates to falsy; the expression is evaluated in command-template space (`${param.*}`, `${context.*}`, `${files.*}`, `${host.*}`, `${dot.path}`) and supports both simple predicates (`cmd: test -f ${files.x}`, `dir-empty ${param.path}`) and boolean literals (`true`, `false`, `0`, `1`). `continue_on_error: true` prevents a step failure from aborting the workflow; the error is logged as a warning and execution proceeds to the next step. Both fields work only on command steps (not confirm steps).

**Docker compose flag forwarding**: Commands of type `service_exec` and `service_run` support a new optional `compose_args: [...]` field — a list of arbitrary docker compose flags (e.g. `-T`, `-d`, `--name`, `--rm`) that are forwarded to the compose command. Template expressions in the list are rendered against the command context. The flags are inserted after policy defaults and run-only flags (e.g. `--no-deps --entrypoint ""`), but before user/workdir/env flags, ensuring user/workdir/env overrides take precedence.

**Unified workdir**: All executable command types (`command`, `script`, `service_exec`, `service_run`) support an optional `workdir` field that sets the working directory for execution. The field is rendered via command templates and resolved relative to the project root. The legacy `cwd` field has been removed entirely. For `type: script` commands, `workdir` affects where the script executes, but `script.path` always resolves relative to the project root (not relative to the `workdir`). The `workdir_from` dot-path field remains exclusive to service runners (`service_exec` and `service_run`); it is now explicitly rejected on other command types.
...
4. **Phase 4 — Commands System** (done): Declarative YAML command definitions in `devbox/commands/`. Six command types: `command`, `devbox`, `script`, `service_exec`, `service_run`, `workflow`. Deploy steps reference commands by ID. Make reduced to lifecycle targets only (`up`, `down`, `stop`, `restart`, `logs`, `deploy`, `deploy-reset`). `make/compose.mk`, `make/service.mk`, `make/deploy.mk` removed. Enhanced: workflow steps support optional `when` conditions (evaluated against command-template space: `${param.*}`, `${context.*}`, `${files.*}`, `${host.*}`) and `continue_on_error` for non-fatal failures; service runners gain `compose_args` to forward docker compose flags before runner-generated flags; unified `workdir` field on all executable types — `cwd` removed, `workdir` applies to `command`, `script`, `service_exec`, `service_run` (for scripts, path always resolves relative to project root), `workdir_from` restricted to service runners.
