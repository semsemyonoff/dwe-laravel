...
- `internal/tpl/` — Go template engine with `Render()`, `EvalCondition()`, custom `FuncMap` (`appURL`, `date`, `datetime`, `base`, `dir`); template funcs available in `${...}` expressions in command definitions
...
Commands may declare a `files:` block to handle external file artefacts (read or write). Each file entry specifies: access mode (`read`, `write`, `read_write`), path or candidates (with glob+match+sort fallback for read modes), and optional env injection. The CLI resolves paths (with template rendering, relative-path normalization to absolute, mkdir for write), exposes resolved paths as `${files.<id>.path}` template variables and env vars, and enforces safe-cleanup semantics: `on_error: remove` deletes files created by the invocation only (not pre-existing files). Templates in `path`, `candidates`, `confirmation_text`, `run`, `cwd`, and `env:` can reference `${files.<id>.path}` after computation but before user confirmation. File ID grammar: `[a-zA-Z_][a-zA-Z0-9_]*` (letters, digits, underscore; no hyphens or dots). See the declarative command definitions in `devbox/commands/` for usage examples (e.g. `db.dump-create`, `db.dump-deploy`).
